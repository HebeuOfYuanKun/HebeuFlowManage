...
public class UserServiceImpl implements UserService {
    ...
    public Result UpdateUserAuth(Map<String, Object> objectMap) {
        ...
        //System.out.println("----------"+DiffAddIds.size()+"-----------"+DiffDesIds.size());
        if(DiffAddIds.size()==0&&DiffDesIds.size()==0){
            ...
            System.out.println("--------------"+
                    "\n"+userMapper.QueryById(id).getUserName());
            authorityService.ClearAuthorityByUserName(userMapper.QueryById(id).getUserName());
        }
        return result;
    }
