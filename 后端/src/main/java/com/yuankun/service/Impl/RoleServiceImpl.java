...
public class RoleServiceImpl implements RoleService {
    ...
    public Result UpdateRoleAuth(Map<String, Object> objectMap) {
        ...
        if(DiffAddIds.size()==0&&DiffDesIds.size()==0){
            ...
            System.out.println("--------------"+
                    "\n"+roleMapper.QueryUserIdByRoleId(id));
            for (Integer userId:roleMapper.QueryUserIdByRoleId(id)) {
                System.out.println(userMapper.QueryById(userId).getUserName());
                authorityService.ClearAuthorityByUserName(userMapper.QueryById(userId).getUserName());
            }
